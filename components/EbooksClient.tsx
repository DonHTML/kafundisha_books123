...
import { WhatsAppIcon, SearchIcon, XIcon } from "@/components/Icons";
import { TabletIcon } from "lucide-react";
import Link from "next/link";
import Image from "next/image";

export default function EbooksClient({ initialProducts }: { initialProducts: any[] }) {
    ...
    return (
        <div className="min-h-screen bg-white pb-32">
            <section className="relative py-24 px-6 overflow-hidden bg-zinc-50">
                <div className="blob bg-purple-100 w-[600px] h-[600px] -top-24 right-0" />
                <div className="relative z-10 mx-auto max-w-7xl text-center">
                    <span className="text-xs font-black uppercase tracking-[0.4em] text-primary">Digital Library</span>
                    <h1 className="mt-4 text-5xl font-black uppercase tracking-tighter text-zinc-900 md:text-8xl">
                        Kafundisha <span className="text-primary italic">eBooks</span>
                    </h1>
                    <p className="mt-8 mx-auto max-w-2xl text-xl text-zinc-500 font-medium mb-12">
                        Downloadable learning materials, study guides, and stories on the go.
                    </p>
                ...
                {filteredProducts.length > 0 ? (
                    <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                        {filteredProducts.map((product: any) => (
                            <div
                                ...
                                className="group flex flex-col justify-between overflow-hidden rounded-[3rem] bg-white border border-zinc-100 p-8 transition-all hover:shadow-2xl hover:shadow-purple-500/10 hover:-translate-y-1"
                            >
                                <div>
                                    <div className={`relative mb-8 aspect-[3/4] overflow-hidden rounded-[2rem] bg-zinc-50 flex items-center justify-center shadow-inner`}>
                                        {product.image_url ? (
                                            <div className="relative h-full w-full p-4">
                                                <Image
                                                    ...
                                                    className="object-contain shadow-2xl rounded-lg transition-transform duration-700 group-hover:scale-105"
                                                />
                                            </div>
                                        ) : (
                                            <TabletIcon className="h-24 w-24 text-zinc-300 opacity-50" />
                                        )}
                                ...
                                <div className="flex flex-col gap-6 mt-8">
                                    ...
                                    <Link
                                        href={`https://wa.me/260977755766?text=Hi! I want to buy the eBook: ${product.name}.`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="group flex items-center justify-center gap-3 w-full rounded-2xl bg-zinc-900 py-4 font-black uppercase tracking-widest text-white transition-all hover:bg-primary shadow-lg"
                                    >
                    ...
                    <div className="py-32 text-center animate-in fade-in duration-500">
                        <div className="mx-auto w-24 h-24 rounded-full bg-zinc-50 flex items-center justify-center text-zinc-300 mb-8">
                            <TabletIcon className="h-12 w-12" />
                        </div>
                        <h3 className="text-4xl font-black uppercase tracking-tighter text-zinc-900 mb-4">Library Empty</h3>
                        <p className="text-xl text-zinc-500 mb-12">No eBooks have been added yet.</p>
                        <button
